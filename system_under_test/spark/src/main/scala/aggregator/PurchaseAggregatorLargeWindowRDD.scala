...
object PurchaseAggregatorLargeWindowRDD {
  case class GemPurchase(key: String, time: Long, price: Double)

  def main(args: Array[String]): Unit = {
    if (args.length < 3) {
      System.err.println("Usage: PurchaseAggregatorLargeWindowRDD <hostname> <port> <file-path>")
      System.exit(1)
    }

    val spark = SparkSession.builder()
      .appName("PurchaseAggregatorLargeWindowRDD")
      .getOrCreate()
    ...
    val windowed = purchases.window(Seconds(60), Seconds(60))
