...
def reserve_nodes_with(num_of_workers, num_of_generators):
    """
    Reserves the needed nodes, given the requested number of workers and generators.
    """
    os.system("preserve -# " + str(num_of_workers + num_of_generators) + " -t 00:15:00")
...
def get_reservation():
    ...
    """
    reservation_result = subprocess.check_output(f"preserve -llist | grep {DDPS_GROUP_USER}", shell=True).split()
    return list(map(lambda x: x.decode('utf8'), reservation_result))


def get_reserved_nodes(num_of_workers, num_of_generators):
    """
    Requests the given amount of nodes, waits till the reservation is complete,
    and returns the allocated nodes
    """
    reserve_nodes_with(num_of_workers, num_of_generators)
